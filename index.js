module.exports = function healerToolbox (dispatch) {
	...
	dispatch.hook('S_PARTY_MEMBER_CHANGE_HP', constants.Protocols.S_PARTY_MEMBER_CHANGE_HP, (event) => {
		...
		let member = partyMembers.find((member) => { return member.playerId == event.playerId })
	...
	dispatch.hook('S_SPAWN_ME', constants.Protocols.S_SPAWN_ME, (event) => { updateAlive(event.gameId, event.alive) })
	dispatch.hook('S_SPAWN_USER', constants.Protocols.S_SPAWN_USER, (event) => { updateAlive(event.gameId, event.alive) })
	dispatch.hook('S_CREATURE_LIFE', constants.Protocols.S_CREATURE_LIFE, (event) => { updateAlive(event.gameId, event.alive) })
	
	dispatch.hook('S_LEAVE_PARTY_MEMBER', constants.Protocols.S_LEAVE_PARTY_MEMBER, ({playerId}) => {
		if (enabled == false)
			return;
		
		partyMembers = partyMembers.filter((member) => { return member.playerId != playerId })
	...
	dispatch.hook('S_ABNORMALITY_BEGIN', constants.Protocols.S_ABNORMALITY_BEGIN, (event) => { return updateEffect(event.target, event.id, false) })
    dispatch.hook('S_ABNORMALITY_REFRESH', constants.Protocols.S_ABNORMALITY_REFRESH, (event) => { return updateEffect(event.target, event.id, false) })
	dispatch.hook('S_ABNORMALITY_END', constants.Protocols.S_ABNORMALITY_END, (event) => { return updateEffect(event.target, event.id, true) })
	...
	function lookupEffect(gameId, effectId, isFinished) {
		...
		dispatch.queryData('/Abnormality/Abnormal@id=?/', [effectId]).then(result => {
			if (!effectsCache.has(effectId))
				effectsCache.set(effectId, {isBuff: result.attributes.isBuff, type: result.attributes.property})
	...
	function effectApplied(gameId, effectId, isFinished) {
		...
		let member = partyMembers.find((member) => { return member.gameId == gameId })
	...
	function updateAlive(gameId, alive) {
		...
		if (alive == false) {
			...
			if (settings.deathMarkers == true) {
				markers.push({color: getMarkerColor(member), target: member.gameId})
			}
		} else {
			if (settings.deathWarning == true) {
			...
			if (settings.deathMarkers == true) {
				markers = markers.filter((marker) => { return marker.target != member.gameId })
			}
		}
	...
	function updateMarkers() {
		dispatch.send('S_PARTY_MARKER', constants.Protocols.S_PARTY_MARKER, { markers: markers })
	}
	...
	function playSound(soundId) {
		dispatch.send('S_PLAY_SOUND', constants.Protocols.S_PLAY_SOUND, { SoundID: soundId });
	}
	...
	function removeWarning(type, member) {
		...
		if (cooldowns != undefined) {
			cooldowns = cooldowns.filter((otherName) => { return name != otherName})
		...
		warnings = warnings.filter((otherName) => { return otherName != name })
	...
	function showWarnings(type) {
		...
		let cooldowns = (cooldownWarnings.get(type) || [])
		names.forEach((name) => { cooldowns.push(name) })
	...
	function getMessage(type, names) {
		...
		switch (type) {
			case constants.WarningTypes.health: return `Heal ${namesString}`
			case constants.WarningTypes.mana: return `Mana ${namesString}`
			case constants.WarningTypes.death: return `Resurrect ${namesString}`
			case constants.WarningTypes.stun: return `Cleanse ${namesString} (Stunned)`
			case constants.WarningTypes.bleed: return `Cleanse ${namesString} (Bleeding)`
			default: return null;
		}
	}
	...
	function getSkillId(type) {
		switch (currentClass) {
			case constants.Classes.mystic:
				switch (type) {
					case constants.WarningTypes.health: return constants.Skills.titanicFavor;
					case constants.WarningTypes.mana: return constants.Skills.arunicRelease; 
					case constants.WarningTypes.death: return constants.Skills.mysticRescurrect; 
					case constants.WarningTypes.stun: 
					case constants.WarningTypes.bleed: return constants.Skills.arunsCleansingTouch; 
					default: return null;
				}
			case constants.Classes.priest:
				switch (type) {
					case constants.WarningTypes.health:return constants.Skills.focusHeal;
					case constants.WarningTypes.mana: return constants.Skills.divineCharge; 
					case constants.WarningTypes.death: return constants.Skills.priestRescurrect; 
					case constants.WarningTypes.stun:
					case constants.WarningTypes.bleed: return constants.Skills.purifyingCircle; 
					default: return null;
				}
			default: return null;
		}
	...
	function getSoundType(name) {
		switch (name) {
			case 'notice': return constants.SoundTypes.notice
			case 'cling1': return constants.SoundTypes.cling1
			case 'cling2': return constants.SoundTypes.cling2
			case 'whisper': return  constants.SoundTypes.whisper
			case 'bell1': return  constants.SoundTypes.bell1
			case 'bell2': return  constants.SoundTypes.bell2
			default: return -1;
		}
	}
